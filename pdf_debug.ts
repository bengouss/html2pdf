        ...
        colorSpaceObj.asArray().forEach((c, index) => {
          if(c instanceof pdflib.PDFName) {
            ...
            if(!colorSpaceRefObj) {
              ...
              if(colorSpaceRefAlternate) {
                colorSpaceName = colorSpaceRefAlternate.toString()
              } else {
                console.warn(`ColorSpace reference ${c.toString()} has no Alternate, using raw data`);
...
const handleImage = async (doc:PDFDocument, pdfRef:PDFRef, pdfObject: PDFRawStream, objectIdx:number, uuid:string, jpegQuality = defaultJpegQuality, maxWidthHeight = 800) => {
  ...
  if(isJPG) {
    ...
    if(width.asNumber() > maxWidthHeight || height.asNumber() > maxWidthHeight) {
      ...
      fs.writeFileSync(`./tmp/${uuid}_${pdfRef.objectNumber}_compressed.jpg`, compressed);
    ...
    fs.writeFileSync(`./tmp/${uuid}_${pdfRef.objectNumber}_compressed.jpg`, compressed.buffer);
...
const compress = async (pdf: Uint8Array | Buffer, uuid:string, jpegQuality?:number, maxWidthHeight?:number) => {
  ...
  for(var i in objects) {
    ...
    if (!!subtype && subtype === pdflib.PDFName.of('Image') && !!type && type === pdflib.PDFName.of('XObject')) {
      const img = await handleImage(doc, pdfRef, pdfObject as PDFRawStream, objectIdx, uuid, jpegQuality, maxWidthHeight);
      if(!img) continue
